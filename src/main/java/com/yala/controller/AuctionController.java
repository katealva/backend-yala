...
public class AuctionController {
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Edita una subasta (precio inicial y fecha de cierre)",
            description = "Solo el dueño del listing y solo si la subasta aún no tiene pujas.")
    public ResponseEntity<ResponseAuctionDTO> update(
            @PathVariable Long id,
            @Valid @RequestBody RequestAuctionUpdateDTO request, Authentication auth) {
        return ResponseEntity.ok(auctionService.update(id, request, auth.getName()));
    }
